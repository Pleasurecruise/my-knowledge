...
Creation writes Chinese R2 content, waits up to 120 seconds for eligible indexing to complete, then publishes D1 metadata. Updates share this deadline. Timeout or incomplete indexing triggers rollback without publication. Translations follow. Daily articles skip indexing; switching to daily removes the item. Rollback checks R2 ETags before derived cleanup.
